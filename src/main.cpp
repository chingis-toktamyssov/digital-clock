...
void setup() {
  ...
  rtc.adjust(DateTime(2025, 9, 30, 0, 16, 0));
}
...
void loop() {
  ...
  int timeDigits[4] = {digHourTens, digHourOnes, digMinuteTens, digMinuteOnes};
  int lengthDigits[4] = {digitLengths[digHourTens], digitLengths[digHourOnes], digitLengths[digMinuteTens], digitLengths[digMinuteOnes]};
  int totalLength = digitLengths[digHourTens]+
  ...
  for (int i = 0; i < 4; i++) {
    ...
    for (int j = 0; j < length; j++) {
      if (i == 0) {
        allDigits[cur + j] = HT[digit][j];
      }
      if (i == 1) {
        allDigits[cur + j] = HO[digit][j];
      }
      if (i == 2) {
        allDigits[cur + j] = MT[digit][j];
      }
      if (i == 3) {
        allDigits[cur + j] = MO[digit][j];
      }
    }
    cur += length;
  }
  lightCustom(allDigits, totalLength);
}